...
void PerimeterGenerator::process_classic()
{
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        if (loop_number > 0 && config->only_one_wall_top && this->upper_slices == nullptr)
            loop_number = 0;
        // hole_wall_loops: derive the max depth for hole (inner) contours.
        // A value of 0 means "use the same as wall_loops". The 0-indexed depth
        // is clamped to [0, loop_number] so it never exceeds the outer count.
        int hole_loop_number = loop_number;
        if (config->hole_wall_loops > 0)
            hole_loop_number = std::min(loop_number, config->hole_wall_loops + surface.extra_perimeters - 1);
        ...
        if (loop_number >= 0) {
            ...
            // hole_wall_loops: discard hole perimeters beyond hole_loop_number.
            // Holes at depth > hole_loop_number are simply dropped so that fewer
            // perimeters surround internal voids than surround external contours.
...
void PerimeterGenerator::process_arachne()
{
    ...
    for (const Surface& surface : all_surfaces) {
        ...
        // TODO (hole_wall_loops / Arachne): The Arachne WallToolPaths engine processes contours
        // and holes together in a single pass using a unified bead count. Per-hole wall limiting
        // via hole_wall_loops is therefore not straightforward here.  For now the setting only
        // takes effect in the classic (non-Arachne) perimeter generator above.
