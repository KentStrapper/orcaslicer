...
enum class EnforcerBlockerType : int8_t {
    ...
    ENFORCER  = 1,
    BLOCKER   = 2,
    // Paint-on support style variants (ported from preFlight slicer).
    // These extend the basic ENFORCER state so that the slicer can later
    // route painted triangles to the correct support algorithm.
    // Values 3 and 4 are safe: the serializer uses a 6-bit extended code
    // for values >= 3 (up to 18), so no existing data is corrupted.
    GRID_ENFORCER    = 3,   // paint-on support: grid style (orange in UI)
    ORGANIC_ENFORCER = 4,   // paint-on support: organic/tree style (green in UI)
    // For the fuzzy skin, we use just two values (NONE and FUZZY_SKIN).
    FUZZY_SKIN = ENFORCER,
    // Maximum is 15. The value is serialized in TriangleSelector into 6 bits using a 2 bit prefix code.
    Extruder1 = ENFORCER,
    Extruder2 = BLOCKER,
    Extruder3 = GRID_ENFORCER,
    Extruder4 = ORGANIC_ENFORCER,
    Extruder5,
    Extruder6,
    Extruder7,
    Extruder8,
