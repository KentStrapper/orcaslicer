...
void ButtonsListCtrl::OnPaint(wxPaintEvent&)
{
    ...
    // Kentstrapper: orange selected button bg instead of blue/teal
    wxColour selected_btn_bg("#FF8800");
...
void ButtonsListCtrl::SetSelection(int sel)
{
    if (m_selection == sel)
        return;
    // Kentstrapper: use orange instead of teal (#009688)
    wxColour selected_btn_bg("#FF8800");
    if (m_selection >= 0) {
    ...
    // Kentstrapper: orange instead of teal (0, 150, 136) — both Hovered and Normal states
    StateColor bg_color = StateColor(
        std::pair{wxColour(255, 136, 0), (int) StateColor::Hovered},
        std::pair{wxColour(255, 136, 0), (int) StateColor::Normal});
    m_pageButtons[m_selection]->SetBackgroundColor(bg_color);
