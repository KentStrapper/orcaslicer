...
void StaticBox::render(wxDC& dc)
{
    ...
    // Use background_color.defaultColor() so the cleared area behind rounded
    // corners matches the button's own color rather than whatever the parent
    // window stored via SetBackgroundColour (which may be a stale teal/accent
    // color set by an older UpdateDarkUI pass).
    wxColour memBg;
    if (background_color.count() > 0) {
        memBg = background_color.defaultColor();
        if (!memBg.IsOk() || memBg.Alpha() == 0)
            memBg = GetBackgroundColour();
    } else {
        memBg = GetBackgroundColour();
    }
    memdc.SetBackground(wxBrush(memBg));
    memdc.Clear();
    {
        wxGCDC dc2(memdc);
        doRender(dc2);
    ...
    if (background_color2.count() == 0) {
        if ((border_width && border_color.count() > 0) || background_color.count() > 0) {
            ...
            if (border_width && border_color.count() > 0) {
                ...
                // No border — pen colour must match background so no fringe appears.
                // Apply the same state-aware + alpha fallback as the brush below.
                wxColour bg = background_color.colorForStates(states);
                if (!bg.IsOk() || bg.Alpha() == 0)
                    bg = background_color.colorForStates(0xFFFF);
                if (!bg.IsOk() || bg.Alpha() == 0)
                    bg = background_color.defaultColor();
                if (!bg.IsOk() || bg.Alpha() == 0)
